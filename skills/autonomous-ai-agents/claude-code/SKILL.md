...
### Content Generation: Reference-File Orchestration Pattern

When delegating content generation (templates, guides, chapters, documentation) to Claude Code via print mode:

1. **Write spec docs first** — create 2-3 markdown files in the workspace:
   - `product-guide.md` — what to build, structure, output requirements
   - `translation-rules.md` — style, terminology, forbidden patterns (critical for cross-cultural adaptation)
   - `source-material-reference.md` — raw source content for context

2. **Write a prompt file** — create `prompt-taskname.txt` with the specific task instructions, file paths, and requirements

3. **Execute via pipe pattern** (most reliable — avoids shell quoting issues):
   ```bash
   cd /path/to/project && cat prompt-taskname.txt | claude -p "Read the reference files first, then execute the task" --allowedTools Read,Write --max-turns 20 --max-budget-usd 4
   ```

4. **Review against checklist** — after Claude Code finishes, spot-check output for rule compliance (forbidden terms, style violations, data integrity)

**Why this works:** Claude Code reads the reference files (product-guide.md, translation-rules.md, etc.) which provide context. The prompt file provides task-specific instructions. Piping via `cat` avoids all shell quoting issues that arise with inline `-p` strings.

**Best for:** Template generation (CSV/Google Sheets), ebook chapters, documentation, guides — any multi-file content generation where you have spec docs and need consistent style enforcement.

**Cost expectations:** ~$0.50 for 10 CSV templates, ~$2-4 for 7 ebook chapters. Set `--max-budget-usd` to prevent runaway spend.

### Print Mode (`-p`) — Non-Interactive (PREFERRED for most tasks)
...
## Content Creation Pipeline (Non-Code Tasks)

Claude Code is also effective for **content creation** — writing guidebooks, templates, playbooks, and structured documents. The same print mode works for prose as well as code, but requires a different handoff pattern.

### When to use
- You need to generate 10+ structured files (CSV templates, markdown chapters) with consistent rules
- The content requires platform-specific knowledge (Discord vs Circle vs Skool) that Claude knows
- You want to enforce strict rules (style guide, banned terms, platform mapping) on generated content
- The task is well-defined enough to specify in a prompt, but large enough to benefit from Claude's context window

### The Three-Document Handoff Pattern

Before calling Claude Code for content creation, write three documents:

1. **product-guide.md** (guidance) — WHAT to build. Product definition, positioning, tiers, file structure.
2. **translation-rules.md** (rules) — HOW. Platform mapping, banned terms, style guide, terminology. Make "forbidden" lists explicit.
3. **review-checklist.md** (review) — How to audit. Run on every file. FAIL = reject.

### Pipe-in Pattern (avoid quote escaping issues on Windows/MSYS)

**DO NOT** put long prompts with complex quotes into the CLI argument. Quote nesting in git-bash/MSYS will fail. Instead:

1. Write the prompt to a text file:
   ```markdown
   write_file(path="project/prompt.txt", content="Read these three files... then create X...")
   ```
2. Pipe the prompt file to Claude Code:
   ```
   terminal(command="cd project && cat prompt.txt | claude -p 'Read the files and execute the task' --allowedTools Read,Write --max-turns 25 --output-format json --max-budget-usd 3", timeout=600)
   ```

### Content Creation Task Structure

```
terminal(command="cd /path/to/project && cat task-prompt.txt | claude -p 'Read the files and execute the task' --allowedTools Read,Write --max-turns N --output-format json --max-budget-usd X", timeout=300)
```

Key flags for content tasks:
- `--allowedTools Read,Write` — restrict to file operations only
- `--max-turns 15-25` — enough for multi-file creation
- `--max-budget-usd 2-5` — cost cap
- `--output-format json` — structured result with session_id, cost, num_turns

### Cost expectations
- 10 CSV templates with sample data: ~$0.48, 18 turns, 2 minutes
- A single chapter (~8 pages): ~$0.10-0.20, 3-5 turns
- Full playbook (7 chapters): ~$1-2
